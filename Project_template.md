...
2. **Фаза выделения микросервисов (Паттерн Strangler Fig):**
   ...
   - Events-Service не включён в диаграмму контейнеров (docs/Container.puml), так как является экспериментальным функционалом, связанным с использованием Kafka.
